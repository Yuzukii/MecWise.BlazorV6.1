...
namespace MecWise.Blazor.Api.Repositories {
    public class FileRepository : GenericRepository<SV_EPF_UPLD_FILE> {
        DBHelper _db;
        public FileRepository(DBHelper db) : base(db) {
            _db = db;
        }

        public int GetNewLineNo(JObject postData) {
            return _db.GetAValue("SELECT ISNULL(MAX(LINE_NO),0) + 1 FROM SV_EPF_UPLD_FILE WHERE KEY_CODE = %s", postData["KEY_CODE"].ToStr()).ToInt();
        }
